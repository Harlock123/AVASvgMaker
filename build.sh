#!/usr/bin/env bash
#
# Builds AVASvgMaker for every supported target.
#
...
# start and a third of the size - but it is NOT a single file. It leaves
...
PROJECT="AVASvgMaker/AVASvgMaker.csproj"
...
# A friendly name for the message about which machine a target needs.
os_name() {
  case "$1" in
    linux) echo "Linux" ;;
    win)   echo "Windows" ;;
...
echo "AVASvgMaker $VERSION - $MODE build on $HOST_OS"
echo
...
for rid in "${TARGETS[@]}"; do
  ...
  if [[ "$MODE" == "aot" ]]; then
    ...
    # the whole bundle before anything runs, and these are the flags that are
    # known to work across the other Avalonia projects here.
  ...
  archive="$OUTPUT/AVASvgMaker-$VERSION-$rid"
